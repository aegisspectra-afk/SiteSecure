-- Atomic workspace-scoped catalog import commit (Task 15B).
-- All create/update rows succeed or the entire call rolls back.
...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE((r->>'_test_force_fail')::boolean, false) THEN
      -- Sentinel for rollback tests: fail after validation, during write phase.
      NULL;
    ELSIF v_action NOT IN ('create', 'update', 'skip') THEN
  ...
  LOOP
    ...
    IF COALESCE((r->>'_test_force_fail')::boolean, false) THEN
      RAISE EXCEPTION 'CATALOG_IMPORT_FORCE_FAIL';
    END IF;
...
REVOKE ALL ON FUNCTION public.catalog_import_commit(uuid, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.catalog_import_commit(uuid, jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.catalog_import_commit(uuid, jsonb) TO service_role;

COMMENT ON FUNCTION public.catalog_import_commit(uuid, jsonb) IS
  'Atomic catalog product create/update for workspace import. Managerial members only.';
